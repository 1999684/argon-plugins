...
本仓库包含为 WordPress Argon 主题开发的三个插件：友链 RSS 展示、友链卡片展示和 Steam 游戏库展示。其他主题没有测试，应该也可以使用。
...
#### 1. 获取 Steam API Key 和 Steam ID

1. 访问 [Steam 开发者网站](https://steamcommunity.com/dev/apikey) 获取 API Key
2. 获取您的 Steam ID（17位数字）：
   - 访问您的 Steam 个人资料页面
   - 在浏览器地址栏中，您会看到类似 `https://steamcommunity.com/id/YOUR_USERNAME/` 或 `https://steamcommunity.com/profiles/YOUR_STEAM_ID/` 的 URL
   - 如果是第一种格式，您需要使用 [SteamID.io](https://steamid.io/) 等工具转换为 17 位数字 ID

#### 2. 配置插件设置

在 WordPress 后台，进入"设置"->"Steam Library"页面，可以设置以下选项：

- **Steam API Key**：您的 Steam API Key
- **Steam ID**：您的 Steam ID（17位数字）
- **每页游戏数**：每次加载显示的游戏数量
- **每行卡片数**：每行显示的游戏卡片数量（1-6之间）
- **缓存时间**：游戏数据缓存的时间（秒），默认为3600秒（1小时）

#### 3. 在页面中显示 Steam 游戏库

使用短代码 `[steam_library]` 在任何页面或文章中显示您的 Steam 游戏库。
...
- 对于 Steam 游戏库插件：
  - 是否正确填写了 Steam API Key 和 Steam ID
  - 您的 Steam 个人资料是否设为公开

### 如何修改显示样式？

所有插件都自带了美观的默认样式，如果您需要自定义样式，可以在您的主题中添加自定义 CSS 来覆盖默认样式，或直接修改插件目录中的 `style.css` 或 `css/style.css` 文件。
...
### Steam 游戏库
- **1.0.0**：初始版本发布，支持卡片式布局和加载更多功能
>>>>>>> 22d198b66acb307c5e8a9599f5dc2fb6721e4040
